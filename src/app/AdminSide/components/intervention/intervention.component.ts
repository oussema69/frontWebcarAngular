...
options:any[]=[]
diagnostic:any[]=[]
  constructor(private router:Router,private interService:InterventionService,private route:ActivatedRoute) { }

  ngOnInit() {
   this.id= this.route.snapshot.params["id"]
   console.log('thid',this.id)
   this.getAll(this.id)
  }
getAll(id:any){
  this.interService.getByReclamation(id).subscribe(res=>{
...
anti: any[] = [];
  ...
  getOptions(type: any, id: any) {
    this.interService.gettacheByTypeId(type, id).subscribe(res => {
      if (res != null) {
        this.options = res;
